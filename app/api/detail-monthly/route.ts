  ...
  try {
    ...
    if (mgFv.length) {
      ...
      if (mappedMg.length) {
        ...
        vals.push(...mappedMg);
        orParts.push(`kode_mix IN (SELECT km.kode_mix FROM portal.kodemix km JOIN portal.hpprsp h ON h.kode = km.kode WHERE h.mg_disney IN (${phs}))`);
      }
      if (includeUnmapped) {
        orParts.push(`kode_mix NOT IN (SELECT km.kode_mix FROM portal.kodemix km JOIN portal.hpprsp h ON h.kode = km.kode WHERE h.mg_disney IS NOT NULL AND h.mg_disney <> '')`);
      }
